function [s,sl,np] = sd_alg_eP2P_l1_inert(np)
...
%        np = alg_param_37b_l2(np);
        ...
        for t = 1:np.t_max
           ...
           for i=1:np.n
                ...
                s.u_tilde{i}(:,t+1) = s.u{i}(:,t+1)+ np.theta*(s.u{i}(:,t+1)-s.u{i}(:,t));
            ...
            for i=1:np.n
                ...
                for j=1:np.n
                    if np.Adj(i,j) == 1
                        ...
                        % Inertial dual variable update
                        s.mu_tr_tilde{i,j}(:,t+1) = s.mu_tr{i,j}(:,t+1) + np.theta*(s.mu_tr{i,j}(:,t+1)-s.mu_tr{i,j}(:,t));
...
%                         dres2(t) = dres2(t) + (s.mu_tr{i,j}(:,t+1) - s.mu_tr{i,j}(:,t))'*(s.mu_tr{i,j}(:,t+1) - s.mu_tr{i,j}(:,t)); 
            ...
            % Inertial primal variable update
            for y=1:np.b
            s.u_DSO_tilde{y}(:,t+1) = s.u_DSO{y}(:,t+1)+ np.theta*(s.u_DSO{y}(:,t+1)-s.u_DSO{y}(:,t));
            ...
            % Inertial dual variable (grid constraints)
            s.lambda_mg_tilde(:,t+1) = s.lambda_mg(:,t+1) + np.theta*(s.lambda_mg(:,t+1)-s.lambda_mg(:,t));
            ...
            for y=1:np.b
                ...
                s.mu_pb_tilde{y}(:,t+1) = s.mu_pb{y}(:,t+1) + np.theta*(s.mu_pb{y}(:,t+1)-s.mu_pb{y}(:,t));
            ...
            s.mu_tg_tilde(:,t+1) = s.mu_tg(:,t+1) + np.theta*(s.mu_tg(:,t+1) - s.mu_tg(:,t));
