function s = projDSO_fbf(s,a_DSO,np,t)
...
for y=1:np.b
    s.u_DSO_tilde{y}(:,t) = z_b{y}(:,end);
    s.p_tg_tilde{y}(:,t) = s.u_DSO_tilde{y}(np.h*2+1:np.h*3,t);
    for zz = 1:length(np.B{y})
        z = np.B{y}(zz);
        s.p_l_tilde{y,z}(:,t) =  s.u_DSO_tilde{y}(np.h*(3+zz-1)+1:np.h*(3+zz),t);
        s.q_l_tilde{y,z}(:,t) =  s.u_DSO_tilde{y}(np.h*(3+length(np.B{y})+zz-1)+1:np.h*(3+length(np.B{y})+zz),t);
    end
end
