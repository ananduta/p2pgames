...
A_eq_dso = cat(1,Ef{:});
A_eq_dso_pinv = pinv(A_eq_dso);%'*np.A_eq_dso)*np.A_eq_dso'; 
...
np.n_eq_dso = c;
np.A_eq_dso_A = sparse(A_eq_dso_pinv*A_eq_dso);
...
for y = 1:np.b
    ...
    for zz = 1:length(np.B{y})
        ...
        a = zeros(1,np.le_u(y));
        a(1,3+zz) = 1;
        np.Spl{y,z} = sparse(kron(a,eye(np.h)));
        
        a = zeros(1,np.le_u(y));
        a(1,3+length(np.B{y})+zz) = 1;
        np.Sql{y,z} = sparse(kron(a,eye(np.h)));
        
        Spq{y} = [Spq{y};np.Spl{y,z};np.Sql{y,z}];
    end
    
    
end
np.Spqall = blkdiag(Spq{:});
