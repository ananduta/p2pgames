function np = alg_param_37b_f(np)
% Step-size selection
% W. Ananduta
% 01/05/2022
...
%     a_all = [a_pi;a_ch;a_ds;a_mg;a_tr;zeros(Ni,1)];
    a_all = [(3 + np.n*np.d_mg + a_add)*ones(4+Ni,1);zeros(Ni,1) ];
    a_ik = diag(a_all);
...
for y=1:np.b
    ...
    a_all = (3 + np.n*np.d_mg + a_add)*ones(size(a_all));
    
    np.A_DSO{y} = sparse(kron(diag(a_all),eye(np.h)));
    
    np.beta_pb(y) = 1/(length(np.B{y})+2*length(np.N_b{y})+a_add);
    

end

np.beta_tg = 0.8/(np.n+np.b+a_add);

np.gamma_mg = 0.8/(np.n+a_add);
