function s = loc_opt_qprog_l1(np,s,t,i)
...
%for i = 1:np.n
    ...
    for h = 1:np.h
        c_1 = c + [0; 0; 0; np.d_mg*(s.sigma_mg(h,t)-pmg_i_t(h)); zeros(2*sum(np.Adj(i,:)),1)]; % Nash eq.
        %c_1 = c + [0; 0; 0; np.d_mg*(s.sigma_mg(h,t)); zeros(2*sum(np.Adj(i,:)),1)];            % Wadrop eq.
        c1 = [c1;c_1];
    end
    
    % linear term associated with p_di
    c2 = np.Sdi{i}'*-s.mu_pb{np.B_n(i)}(:,t);
    
    % linear term associated with p_ch
    c3a = np.Sch{i}'*s.mu_pb{np.B_n(i)}(:,t);
    
    % linear term associated with p_ds
    c3b = np.Sds{i}'*-s.mu_pb{np.B_n(i)}(:,t);
    
    c3 = c3a+c3b;
    
    % linear term associated with grid constraints
    c4 = s.lambda_mg(:,t)'*[np.Smg{i};-np.Smg{i}]+s.mu_tg(:,t)'*np.Smg{i};
    c4 = c4';
