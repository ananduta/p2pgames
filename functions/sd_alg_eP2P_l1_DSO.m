function [s,sl,np] = sd_alg_eP2P_l1_DSO(np)
...
% 13/05/2022
        ...
        for t = 1:np.t_max
           ...
           for i=1:np.n
            ...
            tic
            s1 = locOpt_DSO_DR(s,a_DSO,np,t);
            s = locOpt_DSO_YALMIP(s,a_DSO,np,t);   
            
            for y = 1:np.b
               erDSO(y,1) = norm( s.u_DSO{y}(:,t+1) - s1.u_DSO{y}(:,t+1), inf);
            end
           errorOptDSO=norm(erDSO,inf)
            s.comp_DNO = toc;                       
